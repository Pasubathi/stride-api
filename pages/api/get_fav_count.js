...
export default async function favoriteProductCount(req, res) {
    await cors(req, res);
    switch (req.method) {
        case 'POST':
    ...
    async function favoriteProductCount() {
        try {
            ...
            if(accountDet)
            {
                let favCount = await prisma.favorite_products__c.count({
                    where:{
                        account__id: accountDet.sfid
                    }
                });
                return res.status(200).json({ status: "success", message: "Success", data: favCount })
            }else{
