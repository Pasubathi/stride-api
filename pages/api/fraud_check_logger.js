...
export default async function fraudCheckLogger(req, res) {
    await cors(req, res);
    switch (req.method) {
        case 'POST':
            return fraudCheckLogger();
        default:
            return res.status(405).send({ responseCode: 405, message: `Method ${req.method} Not Allowed` })
    }
    async function fraudCheckLogger() {
        ...
        try {
            ...
            if(accountDet)
            {
                await prisma.document_fraud_check__c.create({
                    data: {
                      response_json__c: String(Response_c),
