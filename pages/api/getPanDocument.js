...
export default async function getPanDocument(req, res) {
    await cors(req, res);
    switch (req.method) {
        case 'POST':
            return getPanDocument();
        default:
            return res.status(405).send({ responseCode: 405, message: `Method ${req.method} Not Allowed` })
    }
    async function getPanDocument() {
        const { sfid } = req.body;
        ...
        try {
            ...
            if(accountDet)
            {
                    ...
                    const getPanImg = await prisma.contentversion.findFirst({
                        where:{
                    ...
                    if(!getPanImg)
                    {
                        ...
                        const getContentData = await getContentPanImages(getPanImg, apiInit);
                        return res.status(200).json({ status: 'success', message: "Success", data: getContentData})
                    }
            } else {
...
async function getContentPanImages(getData, apiInit) {
    return new Promise(async (resolve, reject) => {
        try {
            let imageData = [];
           if(getData)
