...
export default async function addBank(req, res) {
    ...
    async function addBank() {
        const { bank_name, account_number, account_name, ifsc, branch, account_type, bank_sfid, is_active } = req.body;
        ...
        try {
            ...
            if (accountDet) {
               await prisma.bank_account__c.update({
                ...
                return res.status(200).json({ status:"success", message:"Bank account updated successfully"})
            } else {
                return res.status(200).json({ status:"error",message: "Account is not updated" })
            }
        } catch (error) {
