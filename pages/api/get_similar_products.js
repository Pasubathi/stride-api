...
export default async function getSimilarProducts(req, res) {
    await cors(req, res);
    switch (req.method) {
        case 'POST':
    ...
    async function getSimilarProducts() {
        const { category, page, limit, user_sfid } = req.body;
            ...
            if(catDet || subCatDet)
            {
                ...
                if (fetchData) {
                    ...
                    await Promise.all(fetchData.map(async element => {
                        ...
                        let rowData = element;
                            rowData.image_url__c = getContentData;
                            rowData.isFavorite = isFavorite;
                            rowData.product_sub_category__c = subCatDet && subCatDet.name?subCatDet.name:'',
                            rowData.product_category__c = catDet && catDet.name?catDet.name:'',
