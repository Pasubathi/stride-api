        ...
        try {
            ...
            if(productDate)
            {
                ...
                if(fetchData) {
                     ...
                     //   const getContentData = await getContentImages(fetchData.sfid);
                    //    fetchData.images = getContentData;
                        ...
                        const searchDet = await getProductSearch(fetchData);
                    return res.status(200).json({ status: 'success', message: "Success", data: fetchData, search: searchDet })
                } else {
...
async function getContentImages(product_sfid) {
    return new Promise(async (resolve, reject) => {
        try {
            const getImgData = await prisma.product_gallery__c.FindMany({
                where:{
...
async function getProductSearch(productDet) {
    return new Promise(async (resolve, reject) => {
        try {
            let colors  = [];
            let storage = [];
            ...
            const catDet = await prisma.product_subcategory__c.findFirst({
                where:{
                    sfid: productDet.product_subcategory__c
                }
            });
            if(catDet && (catDet.name == "Mobile" ||catDet.name == "Laptop"))
            {
                const colorDet = await prisma.product2.groupBy({
                    by: ['color__c'],
                    where:{
                        product_subcategory__c: productDet.product_subcategory__c,
                        product_category_lu__c: productDet.product_category_lu__c,
                        product_brand__c: productDet.product_brand__c,
                        color__c: {
                            notIn: ['']
                        }
                    }
                });
                const storageDet = await prisma.product2.groupBy({
                    by: ['storage_size__c'],
                    where:{
                        product_subcategory__c: productDet.product_subcategory__c,
                        product_category_lu__c: productDet.product_category_lu__c,
                ...
                const momoryDet = await prisma.product2.groupBy({
                    by: ['memory__c'],
                    where:{
                        product_subcategory__c: productDet.product_subcategory__c,
                        product_category_lu__c: productDet.product_category_lu__c,
                ...
                colors = await retriveColorsData(colorDet);
                storage = await retriveStorageData(storageDet);
                memory = await retriveMemoryData(momoryDet);
                if((colors && colors.length > 0)|| (storage && storage.length > 0) || (memory && memory.length > 0))
                {
                    resolve({status: true, data: {colors, storage, memory}});
                }else{
                    resolve({status: false, data: ''});
                }
            }if(catDet && (catDet.name == "EV" ||catDet.name == "Two Wheelers"))
            {
                const colorDet = await prisma.product2.groupBy({
                    by: ['product_brand__c', 'product_subcategory__c','product_category_lu__c', 'color__c'],
                    where:{
                        product_subcategory__c: productDet.product_subcategory__c,
                        product_category_lu__c: productDet.product_category_lu__c,
                        product_brand__c: productDet.product_brand__c,
                ...
                {
                    resolve({status: true, data: {colors}});
                }else{
                    resolve({status: false, data: ''});
                }
            }else{
...
async function retriveColorsData(colorDet) {
    return new Promise(async (resolve, reject) => {
        try {
            let colors = [];
            await Promise.all(colorDet.map(async element => {
                const productDet = await prisma.product2.findFirst({
                    where:{
                        product_sub_category__c: element.product_sub_category__c,
                ...
                {
                    let obj = {
                        color: productDet.color__c,
                        product: productDet.sfid,
                    }
                    colors.push(obj);
                }
            }));
...
async function retriveStorageData(colorDet) {
    return new Promise(async (resolve, reject) => {
        try {
            let storage = [];
            await Promise.all(colorDet.map(async element => {
                const productDet = await prisma.product2.findFirst({
                    where:{
                        ...
                        product_category__c: element.product_category__c,
                        product_brand__c: element.product_brand__c,
                        storage_size__c: element.storage_size__c,
                    },
                    orderBy: {
                        id: 'desc',
                    },
                ...
                {
                    let obj = {
                        storage: productDet.storage_size__c,
                        product: productDet.sfid,
                    }
                    storage.push(obj);
                }
...
async function retriveMemoryData(colorDet) {
    return new Promise(async (resolve, reject) => {
        try {
            let momory = [];
            await Promise.all(colorDet.map(async element => {
