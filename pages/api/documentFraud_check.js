...
export default async function bureaucheck(req, res) {
    ...
    async function bureauCheck() {
        try {
            ...
            const getdata = await fetch('https://anypoint.mulesoft.com/mocking/api/v1/sources/exchange/assets/e42db8de-d6a7-4bff-871a-d86f2df5a62b/s-edvnz-ocr-api/0.0.7/m/ocr/documentfraudcheck',init).then((response) => response.json())
             .then((response) => {
                  return response;
             });
